...
from .models import RentingService,BookARide,Emergency,Careers
...
def bookRider(request):
    if request.method == 'POST':
        ...
        bookRider = BookARide(firstname=firstname,lastname=lastname,email=email,phone=phone,gender=gender,city=city,pick_up=pick_up,drop_addr=drop_addr)
        bookRider.save()
...
def emergency(request):
    ...
    if request.method == 'POST':
        ...
        emergency = Emergency(firstname=firstname,lastname=lastname,email=email,phone=phone,gender=gender,city=city,pick_up=pick_up,drop_addr=drop_addr)
        emergency.save()
        messages.success(request, 'Thank You for registration')
        return redirect('payment')
...
def rentBike(request):
    if request.method == 'POST':
        ...
        messages.success(request, 'Thank You for registration')
        return redirect('payment')
    return render(request,'rider/rentabike.html')
...
def careersPage(request):
    if request.method == 'POST':
        ...
        careers = Careers(fname=fname,lname=lname,email=email,phone=phone,aadhar=aadhar,curr_address=curr_address,perm_address=perm_address,dlno=dlno)
        careers.save()
        messages.success(request, 'Thank You for registration')
        return redirect('home')
    return render(request,'rider/careers.html')
